...
#define TimerLog(name) std::cout << std::string(#name) << ": " << TimeDiff(name##_start, name##_stop)/1000000.0f << " s" << std::endl
#define TimerStopAndLog(name) struct timeval name##_stop; gettimeofday(&name##_stop,NULL); std::cout << "<-- " + std::string(#name) << " done: " << TimeDiff(name##_start, name##_stop)/1000000.0f << " s" << std::endl
#else
